...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  const { chats, navigate, setSelectedChats, theme, setTheme, user, token, createNewChat, axios, setChats, fetchUserChats, setToken } = useAppContext();
  ...
  return (
    <div className={` ${!isMenuOpen && 'max-md:-translate-x-full'} flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b from-[#2f0952] to-[#000000]/30 border-r border-[#470229]/80 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1`}>
      ...
      {/* button "New Chats" */}
      <button onClick={createNewChat} className="flex justify-center items-center font-semibold w-full py-2 mt-10 text-white bg-gradient-to-r from-[#6d24b2] to-[#8a0b53] text-md rounded-md cursor-pointer ">
      ...
      <div onClick={() => { navigate('/credits'); setIsMenuOpen(false) }} className="flex items-center border border-gray-500 p-0.5 rounded-2xl justify-center gap-2 font-semibold mt-2 cursor-pointer hover:scale-100 transition-all">
      ...
      <div className="flex text-xs items-center justify-center gap-2 font-semibold mt-6  bg-gradient-to-r text-white text-center p-1 rounded-2xl from-[#0d1c8f] to-pink-500 cursor-pointer group">
        ...
        {
          token && <span onClick={logout} className="material-symbols-outlined cursor-pointer">logout</span>
        }
      </div>
